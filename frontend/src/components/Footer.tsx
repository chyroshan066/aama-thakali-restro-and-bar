...
export const Footer = memo(() => {
  ...
  const handleSubscriptionSubmit = useCallback(
    async (data: SubscriptionFormData) => {
      try {
        // This now calls the updated fetch logic above
        const result = await onSubscriptionSubmit(data);
        ...
        // Captures the 'Throw Error' from the utility or network failures
        const errorMessage =
          error instanceof Error
            ? error.message
            : "Something went wrong. Please try again later.";

        showAlert("error", errorMessage, "Subscription Failed");
      }
    },
    [reset, showAlert],
  );
      ...
      >
        <div className="custom-container">
          <div className="footer-top grid-list">
            <div
              className="footer-brand has-before has-after"
              // style={{ "backgroundImage": "url('/images/footer/footer-form-bg.webp')" } as React.CSSProperties}
              style={
                {
                  backgroundImage: "url('/images/footer/footer-form-bg.webp')",
                } as React.CSSProperties
              }
            >
              ...
              <p className="label-1">
                Subscribe us & Get{" "}
                <span className="span">Regular Updates.</span>
              </p>
              <form
