...
//               className="grid gap-x-8 gap-y-10 sm:grid-cols-2 lg:grid-cols-4 relative z-10"
//             >
//               <div className="space-y-2">
//                 <label className="text-[9px] uppercase tracking-[0.2em] text-[#c19977] font-bold">
//                   Item Name
//                 </label>
//                 <input
//                   required
...
//                   placeholder="e.g. Signature Truffle Ramen"
//                 />
//               </div>
...
//                   className="w-full text-[10px] text-gray-500 file:mr-4 file:border file:border-[#c19977]/30 file:bg-transparent file:px-4 file:py-1.5 file:text-[9px] file:uppercase file:tracking-widest file:text-[#c19977] hover:file:bg-[#c19977] hover:file:text-black file:transition-all cursor-pointer"
//                 />
//               </div>
...
//                   }
//                   className="w-full border-b border-white/10 bg-transparent py-2.5 text-sm text-white outline-none focus:border-[#c19977] transition-all placeholder:text-gray-800"
//                   placeholder="Describe the ingredients, texture, and inspiration..."
//                 />
//               </div>
...
//                 <button
//                   type="submit"
//                   className="flex-1 bg-[#c19977] py-3.5 text-[10px] font-bold uppercase tracking-[0.3em] text-black hover:bg-[#d4b580] transition-all shadow-[0_10px_20px_-10px_rgba(193,153,119,0.3)]"
//                 >
...
//                     type="button"
//                     onClick={resetForm}
//                     className="bg-white/5 px-5 py-3.5 text-[11px] text-gray-400 hover:bg-white/10 hover:text-white transition-all border border-white/10"
//                   >
//                     ✕
//                   </button>
//                 )}
...
//                   <article
//                     key={item.id}
//                     className="group relative border border-white/[0.05] bg-zinc-950/40 p-4 transition-all duration-500 hover:border-[#c19977]/40"
//                   >
...
//                           alt={item.name}
//                           fill
//                           className="object-cover opacity-80 grayscale-[0.5] transition-all duration-1000 group-hover:grayscale-0 group-hover:opacity-100 group-hover:scale-105"
//                         />
//                       ) : (
//                         <div className="flex h-full w-full items-center justify-center text-[9px] uppercase tracking-[0.3em] text-zinc-800">
//                           Visual Missing
//                         </div>
//                       )}
...
//                         <span className="text-[9px] uppercase tracking-[0.3em] text-[#c19977] font-bold">
//                           {item.category}
//                         </span>
//                       </div>
//                       <h3 className="font-serif italic text-xl text-white group-hover:text-[#c19977] transition-colors duration-500">
//                         {item.name}
//                       </h3>
...
export default function MenuPage() {
  ...
  // Load menu logic updated to support appending pages
  async function loadMenu(pageNumber = 1, append = false) {
    if (pageNumber === 1) setState({ status: "loading" });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Save failed");
      resetForm();
      // Reset to page 1 after edit/create to see changes clearly
        ...
        <div className="p-6 lg:p-10 relative z-10">
          ...
          <section className="mb-16 border border-white/[0.05] bg-white/[0.01] p-8 backdrop-blur-md relative overflow-hidden group">
            ...
            <form onSubmit={handleSubmit} className="grid gap-x-8 gap-y-10 sm:grid-cols-2 lg:grid-cols-4 relative z-10">
               ...
               <div className="space-y-2"><label className="text-[9px] uppercase tracking-[0.2em] text-[#c19977] font-bold">Item Name</label><input required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="w-full border-b border-white/10 bg-transparent py-2.5 text-sm text-white outline-none" placeholder="e.g. Signature Truffle Ramen" /></div>
               <div className="space-y-2"><label className="text-[9px] uppercase tracking-[0.2em] text-[#c19977] font-bold">Category</label><input required value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} className="w-full border-b border-white/10 bg-transparent py-2.5 text-sm text-white outline-none" placeholder="e.g. Small Plates" /></div>
               <div className="space-y-2"><label className="text-[9px] uppercase tracking-[0.2em] text-[#c19977] font-bold">Price (NPR)</label><input type="number" step="1" required value={form.price} onChange={(e) => setForm({ ...form, price: e.target.value })} className="w-full border-b border-white/10 bg-transparent py-2.5 text-sm text-white outline-none" placeholder="00.00" /></div>
               <div className="space-y-2"><label className="text-[9px] uppercase tracking-[0.2em] text-[#c19977] font-bold">Display Image</label><input type="file" accept="image/*" onChange={(e) => setImageFile(e.target.files?.[0] ?? null)} className="w-full text-[10px] text-gray-500 file:mr-4 file:border file:border-[#c19977]/30 file:bg-transparent file:px-4 file:py-1.5 file:text-[9px] file:uppercase file:text-[#c19977] cursor-pointer" /></div>
               <div className="sm:col-span-2 lg:col-span-3 space-y-2"><label className="text-[9px] uppercase tracking-[0.2em] text-[#c19977] font-bold">Composition</label><input required value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} className="w-full border-b border-white/10 bg-transparent py-2.5 text-sm text-white outline-none" /></div>
               <div className="flex items-end gap-3">
                  <button type="submit" className="flex-1 bg-[#c19977] py-3.5 text-[10px] font-bold uppercase tracking-[0.3em] text-black hover:bg-[#d4b580]">{form.id ? "Sync Changes" : "Confirm Entry"}</button>
                  {form.id && <button type="button" onClick={resetForm} className="bg-white/5 px-5 py-3.5 text-[11px] text-gray-400 border border-white/10">✕</button>}
               </div>
            </form>
          </section>
          ...
          {/* Grid Section */}
          <section>
            {state.status === "loading" && <div className="text-center py-20 text-[10px] uppercase tracking-[0.4em] text-gray-600">Accessing Registry</div>}
            {state.status === "success" && (
              <>
                <div className="grid gap-10 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                  {state.items.map((item) => (
                    <article key={item.id} className="group relative border border-white/[0.05] bg-zinc-950/40 p-4 transition-all">
                      <div className="relative mb-6 aspect-[4/5] w-full overflow-hidden bg-black">
                        {item.image_url ? <Image src={item.image_url} alt={item.name} fill className="object-cover opacity-80" /> : <div className="flex h-full w-full items-center justify-center text-[9px] uppercase tracking-[0.3em] text-zinc-800">Visual Missing</div>}
                      ...
                      <h3 className="font-serif italic text-xl text-white">{item.name}</h3>
                      <p className="text-[12px] leading-relaxed text-gray-500 line-clamp-3">{item.description}</p>
                      <div className="mt-6 flex items-center justify-between border-t border-white/[0.05] pt-5">
